...
if(NOT WIN32)
	message(FATAL_ERROR "TestGtpuTelemetry.cmake currently supports Windows only")
endif()
...
[void]$client.Send($bytes, $bytes.Length, '127.0.0.1', 2152)
$client.Dispose()
]=])
...
if(NOT SEND_RESULT EQUAL 0)
	...
	message(FATAL_ERROR "Failed to send GTP-U demo datagram: ${SEND_ERROR}")
endif()

execute_process(
	COMMAND "${RUNTIME_CLI_EXE}" "iface_status Gn" 20
	WORKING_DIRECTORY "${SOURCE_DIR}"
...
if(NOT STATUS_RESULT EQUAL 0)
	message(FATAL_ERROR "Failed to query Gn interface status via CLI: ${STATUS_ERROR}")
endif()
...
if(NOT CLI_OUTPUT MATCHES "Interface Gn status:")
	message(FATAL_ERROR "Expected Gn interface status in CLI output. Output: ${CLI_OUTPUT}")
endif()
if(NOT CLI_OUTPUT MATCHES "Effective Bind: 127\.0\.0\.1:2152")
	message(FATAL_ERROR "Expected effective GTP-U bind IP in CLI output. Output: ${CLI_OUTPUT}")
endif()
...
if(NOT CLI_OUTPUT MATCHES "- Name: Gn")
	message(FATAL_ERROR "Expected Gn endpoint telemetry entry in state output. Output: ${CLI_OUTPUT}")
endif()
...
if(NOT LOG_CONTENT MATCHES "Interface endpoint Gn ready on 127\.0\.0\.1:2152")
	message(FATAL_ERROR "Missing GTP-U endpoint startup log entry")
endif()
if(NOT LOG_CONTENT MATCHES "Interface endpoint Gn received 6 bytes from 127\.0\.0\.1")
	message(FATAL_ERROR "Missing GTP-U receive log entry")
endif()
