...
if(NOT WIN32)
	message(FATAL_ERROR "TestS6aWatchdog.cmake currently supports Windows only")
endif()
...
execute_process(
	COMMAND "${S6A_DIAMETER_CLIENT_EXE}" 15 watchdog
	WORKING_DIRECTORY "${SOURCE_DIR}"
...
if(NOT SEND_RESULT EQUAL 0)
	message(FATAL_ERROR "Failed to send S6a CER+DWR: ${SEND_OUTPUT} ${SEND_ERROR}")
endif()
...
if(NOT CLI_OUTPUT MATCHES "Last Message[ ]*:[ ]+Device-Watchdog-Request \\(DWR\\)")
	message(FATAL_ERROR "Expected DWR as last message in CLI output. Output: ${CLI_OUTPUT}")
endif()
if(NOT CLI_OUTPUT MATCHES "Last Detail[ ]*:[ ]+origin_host=mme\\.vepc\\.local")
	message(FATAL_ERROR "Expected DWR origin_host detail in CLI output. Output: ${CLI_OUTPUT}")
endif()
...
if(NOT LOG_CONTENT MATCHES "Parsed Diameter header from 127\\.0\\.0\\.1: command=Device-Watchdog-Request \\(DWR\\)")
	message(FATAL_ERROR "Missing DWR parse log entry")
endif()
if(NOT LOG_CONTENT MATCHES "Sent Diameter response to 127\\.0\\.0\\.1: command=Device-Watchdog-Answer \\(DWA\\)")
	message(FATAL_ERROR "Missing DWA response log entry")
endif()
