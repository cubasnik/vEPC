...
if(NOT WIN32)
	message(FATAL_ERROR "TestS6aInsertSubscriberData.cmake currently supports Windows only")
endif()
...
execute_process(
	COMMAND "${S6A_DIAMETER_CLIENT_EXE}" 15 insert
	WORKING_DIRECTORY "${SOURCE_DIR}"
...
if(NOT SEND_RESULT EQUAL 0)
	message(FATAL_ERROR "Failed to send S6a CER+IDR: ${SEND_OUTPUT} ${SEND_ERROR}")
endif()
...
if(NOT CLI_OUTPUT MATCHES "Last Message[ ]*:[ ]+Insert-Subscriber-Data-Request \\(IDR\\)")
	message(FATAL_ERROR "Expected IDR as last message in CLI output. Output: ${CLI_OUTPUT}")
endif()
if(NOT CLI_OUTPUT MATCHES "Last Detail[ ]*:[ ]+imsi=001010123456789")
	message(FATAL_ERROR "Expected IDR imsi detail in CLI output. Output: ${CLI_OUTPUT}")
endif()
...
if(NOT LOG_CONTENT MATCHES "Parsed Diameter header from 127\\.0\\.0\\.1: command=Insert-Subscriber-Data-Request \\(IDR\\)")
	message(FATAL_ERROR "Missing IDR parse log entry")
endif()
if(NOT LOG_CONTENT MATCHES "Sent Diameter response to 127\\.0\\.0\\.1: command=Insert-Subscriber-Data-Answer \\(IDA\\)")
	message(FATAL_ERROR "Missing IDA response log entry")
endif()
