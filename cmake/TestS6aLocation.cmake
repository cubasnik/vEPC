...
if(NOT WIN32)
	message(FATAL_ERROR "TestS6aLocation.cmake currently supports Windows only")
endif()
...
execute_process(
	COMMAND "${S6A_DIAMETER_CLIENT_EXE}" 15 location
	WORKING_DIRECTORY "${SOURCE_DIR}"
...
if(NOT SEND_RESULT EQUAL 0)
	message(FATAL_ERROR "Failed to send S6a CER+ULR: ${SEND_OUTPUT} ${SEND_ERROR}")
endif()
...
if(NOT CLI_OUTPUT MATCHES "Last Message[ ]*:[ ]+Update-Location-Request \\(ULR\\)")
	message(FATAL_ERROR "Expected ULR as last message in CLI output. Output: ${CLI_OUTPUT}")
endif()
if(NOT CLI_OUTPUT MATCHES "Last Detail[ ]*:[ ]+imsi=001010123456789")
	message(FATAL_ERROR "Expected ULR imsi detail in CLI output. Output: ${CLI_OUTPUT}")
endif()
...
if(NOT LOG_CONTENT MATCHES "Parsed Diameter header from 127\\.0\\.0\\.1: command=Update-Location-Request \\(ULR\\)")
	message(FATAL_ERROR "Missing ULR parse log entry")
endif()
if(NOT LOG_CONTENT MATCHES "Sent Diameter response to 127\\.0\\.0\\.1: command=Update-Location-Answer \\(ULA\\)")
	message(FATAL_ERROR "Missing ULA response log entry")
endif()
